...
Бедрената глава е приблизително сферична, покрита с ставен хрущял с изключение на fovea capitis (централното вдлъбнатие за ligamentum teres). Кръвоснабдяването на главата се осъществява главно от медиалната феморална циркумфлексна артерия (нейният дълбок клон, изкачващ се задно по шийката на бедрото, „ретинакуларните съдове“), с по-малък принос от латералната феморална циркумфлексна артерия и съдовете на ligamentum teres (последните с по-голямо значение при деца). Ретинакуларните съдове са уязвими при луксация на тазобедрената става и фрактура на шийката на бедрото, което обяснява високата честота на аваскуларна некроза при тези увреждания.

<aside class="callout callout-pearl">
  <span class="callout-label">Pearl</span>
  <p>Перфузията на бедрената глава е доминирана от медиалната феморална циркумфлексна артерия — дълбокият й клон се изкачва задно по шийката на бедрото като ретинакуларните съдове. Луксацията на ТБС и фрактурата на шийката на бедрото нарушават именно тези съдове, поради което АВН е страховитото усложнение и на двете.</p>
</aside>
...
<aside class="callout callout-pearl">
  <span class="callout-label">Pearl</span>
  <p>Увреждането на седалищния нерв усложнява 10-20% от задните луксации на ТБС, класически на перонеалния клон (отпадане на стъпалото) при запазена тибиална функция. Документирайте преди репозицията, за да не се припише пост-репозиционна пареза на маневрата.</p>
</aside>

## Лечение на луксацията на тазобедрената става

Луксацията на тазобедрената става е ортопедична спешност поради времезависимия риск от аваскуларна некроза. Класическото учение е, че репозицията в рамките на 6 часа съществено намалява риска от аваскуларна некроза, докато забавянето над 12 часа предсказва висока честота на аваскуларна некроза (до 50% и повече).

<aside class="callout callout-pearl">
  <span class="callout-label">Pearl</span>
  <p>Луксацията на ТБС е време-зависима ортопедична спешност: репозирайте в рамките на 6 часа за минимизиране на АВН; забавяне над 12 часа предсказва честота на АВН до 50% и повече. Часовникът тръгва от травмата, не от постъпването.</p>
</aside>
...
<aside class="callout callout-pitfall">
  ...
  <p>КТ след репозицията е задължителна при всяка луксация на ТБС — не е по избор. Тя открива интраартикуларни свободни тела (показание за артротомия), окултна фрактура на задната стена, увреждане на главата на бедрото и неконцентрична репозиция (разширяване на ставната цепка от интерпонирани меки тъкани). Изпращането на пациента от спешното с неконцентрична репозиция е път към ранна артроза.</p>
</aside>
...
<aside class="callout callout-pearl">
  <span class="callout-label">Pearl</span>
  <p>Фрактури на бедрената глава по Pipkin — ключът: отношението към fovea определя релевантността спрямо натоварващата повърхност. I (под fovea, нерядко може да се резецира), II (над fovea, натоварваща повърхност, изисква анатомична фиксация), III (глава + шийка, много висок риск от АВН), IV (глава + acetabulum, координирана репозиция). Fovea е анатомичната ос на класификацията.</p>
</aside>
...
Фрактура на задната стена: Най-честият елементарен модел (над 25% от ацетабуларните фрактури). Механизмът е типично „dashboard“ травма с флектирана тазобедрена става. Увреждането засяга задната стена на acetabulum, нерядко с асоциирана задна луксация. Оценката на стабилността зависи от засегнатия процент от задната стена (под 20% обичайно стабилна, над 40% нестабилна, в междинния диапазон се налага стрес-изследване).
...
<aside class="callout callout-pearl">
  <span class="callout-label">Pearl</span>
  <p>Структура на Letournel в пет плюс пет: елементарни модели — задна стена (най-чест, ~25%), задна колона, предна стена, предна колона, напречен. Асоциираните модели ги комбинират — T-образен, задна колона + стена, напречен + задна стена (най-чест асоцииран), предна колона + задна хемитрансверзала и фрактура на двете колони (единственият модел, при който нито един ацетабуларен фрагмент не остава прикрепен към илиачното крило — търсете „spur sign“ на обтураторната наклонена).</p>
</aside>
...
<aside class="callout callout-pearl">
  <span class="callout-label">Pearl</span>
  <p>Критерии на Matta за качество на ацетабуларната репозиция: анатомично ≤1 mm, непълно 2-3 mm, лошо >3 mm. Класическите данни на Letournel — 80% добри/отлични резултати при анатомична репозиция срещу 30% при непълна. Качеството на репозицията е единичният най-силен предиктор на дългосрочния резултат.</p>
</aside>
...
<aside class="callout callout-pitfall">
  <span class="callout-label">Pitfall</span>
  <p>Corona mortis („коронката на смъртта“) е анастомотичен съд между обтураторната и външната илиачна система, преминаващ горния пубисен рамус, налична при приблизително 1 от 3 пациенти. Срещната при илиоингвиналния или модифицирания достъп по Stoppa, увреждането й може да доведе до катастрофален кръвоизлив в трудно достъпно място — идентифицирайте, лигирайте и прекъснете я проактивно.</p>
</aside>
...
Ацетабуларните фрактури при възрастната популация имат съществено различен характер от тези при по-младите — обичайно нискоенергиен механизъм, по-чести модели на предна колона или предна колона с задна хемитрансверзала, нерядко с импакция на купола („Gull sign“, медиална импакция на ацетабуларния купол в таза). Лечението е предизвикателно поради остеопоротичното качество на костта, честите съпътстващи заболявания и високия процент неуспех на откритата репозиция и вътрешна фиксация
...
Ацетабуларните фрактури при деца са рядкост и имат съществено различен вид поради отворения triradiate cartilage. Уврежданията на triradiate cartilage (наподобяващи класификацията на Salter-Harris) могат да доведат до спиране на растежа и прогресираща ацетабуларна дисплазия с дългосрочни последици за развитието на тазобедрената става. Лечението е като правило консервативно, с оперативна интервенция, запазена за разместени модели. Ортопедичният хирург трябва да отчита растежните последици при планиране на всяка оперативна интервенция в близост до triradiate cartilage.

## Постоперативно лечение

Постоперативното лечение включва профилактика на венозна тромбемболия (висок риск в тази популация, вж. Тема Травматология-7), ранна мобилизация на тазобедрената става за предотвратяване на скованост (обичайно от деня след операцията), защитено натоварване за 8 до 12 седмици (обичайно контактно или частично натоварване в зависимост от стабилността на фиксацията) и профилактика на хетеротопна осификация при пациенти с разширени достъпи или висок рисков профил.

## Източници и класическа литература

*Treatise on Dislocations and Fractures of the Joints* на Sir Astley Cooper (1822) дава основополагащото описание на травматичната луксация на тазобедрената става и хирургичната анатомия на репозицията; четиритенаправлена класификация на неговите наследници (Bigelow 1869, Thompson 1969) почива на субстрата на Cooper. Малгень (1847) разширява анализа към ацетабуларните фрактури, предхождайки модерната класификация на Letournel (1974). Класификацията на Pipkin за фрактурите на феморалната глава (1957) рафинира линията, която минава от Birkett (1869) и Hueter.

- **Astley Cooper** (1832 изд.): *A Treatise on Dislocations and Fractures of the Joints*. · [Факсимиле](https://archive.org/details/treatiseondisloc00coop_0)
- **Малгень** (1847): *Traité des fractures et des luxations*, том 2 — ацетабуларни фрактури. · [Факсимиле](https://archive.org/details/BIUSante_32985x02)
- **Хофа** (1905): *Lehrbuch der orthopädischen Chirurgie* — увреди на тазобедрената става. · [Факсимиле](https://archive.org/details/lehrbuchderortho00hoff)
